    ...
    if (urlStringBuilder != null){
      ...
      try{
        ...
        try {
          ...
          batchOperations.addAll(Utils.quoteJsonToContentVals(getResponse));
          if (batchOperations.size() > 0)
            mContext.getContentResolver().applyBatch(QuoteProvider.AUTHORITY, batchOperations);
          else {
              EventBus.getDefault().post(new InternalMassage(mContext.getString(R.string.stock_not_found)));
          }
